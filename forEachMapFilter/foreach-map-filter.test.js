...
describe("#find", function () {
  it("returns the first value found in an array", function () {
      expect(find([1, 2, 3, 4, 5], 3)).toEqual(3);
  });
});

  it("returns undefined if the value is not found", function () {
      expect(find([1, 2, 3, 4, 5], 10)).toEqual(undefined);
  });
});

describe("#filterByValue", function () {
  var arr = [
      {first: "Elie", last: "Schoppik", isCatOwner: false},
      {first: "Tim", last: "Garcia", isCatOwner: true},
      {first: "Matt", last: "Lane", isCatOwner: false},
      {first: "Colt", last: "Steele", isCatOwner: true}
  ];

  it("returns an array of objects with the specified key and value", function () {
      expect(filterByValue(arr, 'isCatOwner')).toEqual([
          {first: "Tim", last: "Garcia", isCatOwner: true},
          {first: "Colt", last: "Steele", isCatOwner: true}
      ]);
  ...
  it("returns an empty array if the value is not found", function () {
      expect(filterByValue(arr, 'isDogOwner')).toEqual([]);
  });
});
